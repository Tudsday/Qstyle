<?php
/**
 * Qtpl 模板引擎 支持 PHP 5.6 及以上版本。
 * @category: 分享工作室;
 * @var: Qstyle5.class.php;
 * @version: Qstyle 8.5.0 2024-06-12;
 */

class QStyle{

    protected $Cnf_suffix = '.html';
    protected $Cnf_cacheDir = '';
    protected $Cnf_tplDir = array();
    protected $Cnf_autoDir = array();

    protected $lang  = array();
    protected $var_assign = array();
    protected $html_replace = array();
    protected $rwcount = 0;

    private $rwtplfile = array();
    private $debugs = array();
    private $find_Dirs = array();
    private $Cnf_update = false;
    private $Cnf_debug = false;
    private $print_html = '';
    private $Cnf_enkey = '';
     ...
     */
    public function __construct($update = false, $debug = false){
        $this->conf(self::CNF_UPDATE, $update);
        $this->conf(self::CNF_DEBUG, $debug);
        $this->Cnf_suffix  = '.' . ltrim($this->Cnf_suffix, '.\\//');

        $this->debug('construct', (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ''));
    }
    ...
    public function clear(){
        ...
        foreach ($files as $file) {
            if (strlen($file) >= 64 && substr($file, -4) === '.php') {
                @unlink($file);
            }
        }
    }
    ...
    public function conf($key, $value){
        ...
        if(is_array(isset($this->$resolvedKey) ? $this->$resolvedKey : null)){
            if(!is_array($value))
                $value = array($value);
            $value = array_merge(array_reverse($value), $this->$resolvedKey);
        }elseif(is_bool(isset($this->$resolvedKey) ? $this->$resolvedKey : null)){
            $value = (bool) $value;
        }elseif(is_int(isset($this->$resolvedKey) ? $this->$resolvedKey : null)){
            $value = (int) $value;
        }else{
            if(is_array($value)){
                // array_key_first 替代: 取第一个键
     ...
     * 支持两种调用方式：
     *   - 数组方式：assign(array('title' => 'Hello', 'name' => 'World'))
     ...
     */
    public function replace($key, $idx = '<>'){
        if(is_array($key)){
            foreach($key as $k => $v){
                if($idx === '<' || $idx === '<>'){
     ...
     */
    protected function enhtml($html){
        // PHP 5.6 没有 ENT_SUBSTITUTE，使用 ENT_QUOTES
        return str_replace(' ', '&nbsp;', htmlspecialchars($html, ENT_QUOTES, 'UTF-8'));
    }
     ...
     */
    protected function parse_tpl($html){
        $html = trim($html);
        if ($html === '')
            return '';
        ...
        $normalizeConstantArrayExpression = function ($expression) {
            $expression = trim($expression);
            $result = preg_replace_callback(
                '/\[(\s*[A-Za-z_\x80-\xff][A-Za-z0-9_\x80-\xff]*\s*)\]/u',
                function ($matches) {
        ...
        $compileValue = function ($value) use ($normalizeExpression) {
            $value = trim($value);
            if($value === ''){
                return "''";
            }
        ...
        $compileEcho = function ($expression) use ($normalizeExpression, $normalizeConstantArrayExpression, $compileValue) {
            $expression = trim($expression);
            ...
            if(count($parts) >= 2){
                ...
                return "<?php echo (((isset({$baseExpression}) ? {$baseExpression} : '' )? {$trueValue} : {$falseValue}));?>";
            }

            if(count($parts) === 1){
                $defaultValue = $compileValue($parts[0]);
                return "<?php echo (isset({$baseExpression}) ? {$baseExpression} : {$defaultValue});?>";
            }

            if(preg_match('/^([A-Za-z_\x80-\xff][A-Za-z0-9_\x80-\xff]*)(?:\[.*\])?$/u', $baseExpression, $matches) === 1 && strpos($baseExpression, '->') === false){
                $constant = $matches[1];
                return "<?php echo (defined('{$constant}') ? (isset({$baseExpression}) ? {$baseExpression} : '') : '');?>";
            }

            return "<?php echo (isset({$baseExpression}) ? {$baseExpression} : '');?>";
        };

        $compileStaticAccess = function ($expression) {
            $expression = trim($expression);
            if($expression === ''){
                return '';
            }
        ...
        $compileLoad = function ($expression) use ($normalizeExpression) {
            $expression = trim($expression);
            if($expression === ''){
                return '';
            }
            ...
            $loadTarget = $expression[0] === '$' ? ("(isset(" . $normalizeExpression($expression) . ") ? " . $normalizeExpression($expression) . " : '')") : var_export($expression, true);
            return "<?php \$this->load({$loadTarget});?>";
        };

        $compileLang = function ($expression) use ($compileValue) {
            $expression = trim($expression);
            if($expression === ''){
                return '';
            }
            ...
            if($compiledParts === array()){
                $compiledParts[] = $compileValue($expression);
            }

            return "<?php echo (\$this->getlang(array(" . implode(', ', $compiledParts) . ")));?>";
        };

        $compileLoopReceiver = function ($receiver) use ($normalizeExpression) {
        ...
        $resolveLoadsTarget = function ($expression) {
            $expression = trim($expression);
            if($expression === ''){
                return '';
            }

            if(($expression[0] === '\'' && substr($expression, -1) === '\'') || ($expression[0] === '"' && substr($expression, -1) === '"')){
            ...
            if($expression[0] === '$' && preg_match('/^\$+([A-Za-z_\x80-\xff][A-Za-z0-9_\x80-\xff]*)$/u', $expression, $matches) === 1){
                $key = $matches[1];
                $value = isset($this->var_assign[$key]) ? $this->var_assign[$key] : (isset($GLOBALS[$key]) ? $GLOBALS[$key] : null);
        ...
        $resolveBlockName = function ($expression) {
            $expression = trim($expression);
            if($expression === ''){
                return '';
            }
        ...
        $normalizeVariableVariableSyntax = function ($expression) {
        ...
        $isSafeReadExpression = function ($expression) {
            $expression = trim($expression);
        ...
        if(isset($this->html_replace['<']) ? $this->html_replace['<'] : null){
        ...
        $matchList = isset($matches[1]) ? $matches[1] : array();
        foreach($matchList as $index => $expression){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) {
                ...
                if($statement !== '' && substr($statement, -1) !== ';'){
                    $statement .= ';';
                }
        ...
        $php = preg_replace_callback(
            '/\{lang\s+(.+?)\s*\}/isu',
            function ($matches) use ($compileLang) {
                return $compileLang($matches[1]);
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) use (&$blockTemplates) {
        ...
        // 调用仅支持变量模式：{block $xxx}。
        if($blockTemplates !== array()){
            $php = preg_replace_callback(
                '/\{block\s+(\$+[A-Za-z_\x80-\xff][A-Za-z0-9_\x80-\xff]*)\}/iu',
                function ($matches) use ($blockTemplates, $resolveBlockName) {
                    $name = $resolveBlockName(isset($matches[1]) ? $matches[1] : '');
                    return $name !== '' ? (isset($blockTemplates[$name]) ? $blockTemplates[$name] : '') : '';
                },
                $php
            );
            if($php === null){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) use ($compileStaticAccess) {
                return $compileStaticAccess($matches[1]);
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) use ($compileEcho) {
                $name = $matches[1];
                // 保留流程关键字，不在这里转成常量输出。
                if(in_array(strtolower($name), array('else'), true)){
                    return $matches[0];
                }
        ...
        $php = preg_replace_callback(
            '/\{if\s+(.+?)\}/isu',
            function ($matches) use ($normalizeExpression) {
                return '<?php if(' . $normalizeExpression($matches[1]) . '){ ?>';
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) use ($normalizeExpression) {
                return '<?php }elseif(' . $normalizeExpression($matches[1]) . '){ ?>';
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace('/\{else\}/iu', '<?php }else{ ?>', $php);
        if($php === null){
            $php = '';
        }
        $php = preg_replace('/\{\/if\}/iu', '<?php } ?>', $php);
        if($php === null){
            $php = '';
        }
        ...
        $php = preg_replace_callback(
            '/\{loop\s+([^\{\}\n]+?)\s+(\$[^\s\}]+)\s+(\$[^\s\}]+)\}/iu',
            function ($matches) use ($normalizeExpression, $compileLoopReceiver) {
                $keyVar = $compileLoopReceiver($matches[2]);
                $valVar = $compileLoopReceiver($matches[3]);
                return '<?php foreach($this->_A(isset(' . $normalizeExpression($matches[1]) . ') ? ' . $normalizeExpression($matches[1]) . ' : array()) as ' . $keyVar . ' => ' . $valVar . '){ ?>';
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace_callback(
            ...
            function ($matches) use ($normalizeExpression, $compileLoopReceiver) {
                ...
                return '<?php foreach($this->_A(isset(' . $normalizeExpression($matches[1]) . ') ? ' . $normalizeExpression($matches[1]) . ' : array()) as ' . $valVar . '){ ?>';
            },
            $php
        );
        if($php === null){
        ...
        $php = preg_replace('/\{\/loop\}/iu', '<?php } ?>', $php);
        if($php === null){
            $php = '';
        }
        ...
        $php = preg_replace_callback(
            '/\{\s*(\$(?:(?:\{[^{}]+\})|[^{}])+?)\s*\}/isu',
            function ($matches) use ($compileEcho, $isSafeReadExpression, $normalizeVariableVariableSyntax) {
                $expression = $normalizeVariableVariableSyntax(trim($matches[1]));
                if(strpos($expression, ';') !== false || preg_match('/(^|[^!<>=])=(?!=)/', $expression) === 1){
                    return $matches[0];
                }

                if(true){
                    $parts = array_map('trim', explode('|', $expression));
                    $baseExpression = isset($parts[0]) ? $parts[0] : '';
     ...
     */
    public function print($tpl, $fkey = ''){
        $this->print_html = '1';
        $filecode = $this->load($tpl, $fkey);
        // 判断.php结尾
        if(substr($filecode, -4) === '.php'){
     ...
     */
    public function load($tpl, $fkey = ''){
        if($this->Cnf_debug && (isset($_GET['print']) ? $_GET['print'] : '')){
            $_GET['print'] = '';
            return $this->print($tpl, $fkey);
        }

        if(!(isset($this->debugs['Qextract']) ? $this->debugs['Qextract'] : '')){
            foreach($GLOBALS as $key => $value){
                // 将用户定义的变量释放
                if(!in_array($key, array('GLOBALS','_SERVER','_GET','_POST','_FILES','_COOKIE','_SESSION','_REQUEST','_ENV','argv','argc'), true)){
            ...
            if(!$this->Cnf_cacheDir){
                ...
                if($basedir !== '' && strpos($basedir, $temdir) !== false){
                    $temdir = '';
                }
            ...
            $this->Cnf_enkey = hash('sha256',(isset($this->Cnf_enkey) ? $this->Cnf_enkey : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '')).md5_file(__FILE__));
        }
        ...
        if($fkey && $this->rwtplfile['tpl']){
            ...
            $tplcode = '';
            if($tplContent){
                if( strpos($fkey, '#') === 0 || strpos($fkey, '.') === 0 ){
    ...
    public function __destruct(){
        ...
        $cntarr = (new ReflectionClass('QStyle'))->getConstants();
        $contval = array();
        foreach($cntarr as $key => $value){
            $contval[$value] = isset($this->{$value}) ? $this->{$value} : $value;
        }
     ...
     */
    protected function extractHtmlContentBySelector($html, $selector){
     ...
     */
    protected function xpathLiteral($value){
    ...
    protected function loads($tpl){
        ...
        if(strpos($tpl, './') === 0 && strpos($tpl, "\n") === false){
            $tpl = trim($tpl, './');
        }

        $tem = $this->autofile($tpl,'tpl');
        $this->debug('loads_'.$tpl, isset($tem['tpl']) ? $tem['tpl'] : '');
        if($tem['tpl']){
    ...
    /**
     * 设置语言包条目。
     * 支持单条设置（setlang('key', 'value')）和批量设置（setlang(array('key1' => 'value1', 'key2' => 'value2')）两种用法。
     * @param  array|string $key 键名或键值对数组
     * @param  mixed $val      可选，单条设置时的值
     */
    public function setlang($key, $val = null){
        if(is_string($key)){
            $key = array($key => $val);
        }
    ...
    /**
     * 获取语言包条目。
     * 支持单条获取（getlang('key')）和批量获取（getlang(array('key1', 'key2')）两种用法。
     ...
     * @return array|string 模板模式返回 array('tpl'=>路径, 'cache'=>缓存路径, 'isupdate'=>是否需更新)；
     *                      自动搜索模式返回文件完整路径或空字符串
     */
    protected function autofile($filename, $GetType ='auto'){
        // 凡是有空格,换行, 都不当成文件名, 直接返回原字符串. 只有纯文件名才进行搜索.
        if(preg_match('/\s/', $filename) === 1){
            $this->debug('autostr-'.microtime(true), strlen($filename));
            return $GetType === 'auto' ? '' : array('tpl' => '', 'cache' => '', 'isupdate' => false);
        }
        ...
        if ($GetType !== 'auto') {
            ...
            if(strpos($filename, '/') !== false){
                $filepath = $filename;
                if(!is_file($filepath)){
                    $filenameHeadParts = explode('.', $filename, 2);
                    $filenameHead = isset($filenameHeadParts[0]) ? $filenameHeadParts[0] : $filename;
            ...
            if($filepath){
                ...
                $filenameHeadParts = explode('.', basename($filename), 2);
                $filenameHead = isset($filenameHeadParts[0]) ? $filenameHeadParts[0] : basename($filename);
                $cachefile = $this->Cnf_cacheDir . '/' .$filenameHead.'_'.$fmd5. '.php';
                if(!is_file($cachefile) || $this->Cnf_update)
                   $isupdate = true;
            }

            $ret = array('tpl' => $filepath, 'cache' => $cachefile, 'isupdate' => $isupdate);
     ...
     *                          文件打开失败返回 false
     */
    protected function preg__file($path, $strData='*********'){
        $mode = $strData !== '*********' ?'wb+':'rb';
